<?php
		...
		$_passwords = FALSE)
	{
		...
		{
			if(!stringContains("CONCAT", $_columns)) $_columns = explode(",", trim($_columns));
			else $onlyOneColumn = $_columns;
		}
		...
		else
		{
			while($row = $result->fetch_assoc())
			{
				...
				{
					foreach($_columns as $column) $return[$id][$column] = $row[$column];
				}
			}
		}
		if(!$_passwords)
		{
			if(is_array($return))
			{
				if(isset($return["password"])) unset($return["password"]);
				if(is_array(end($return)))
				{

					$tmp = array();
					foreach ($return as $key => $value) {
						$tmp[$key] = $value;
						if(isset($tmp[$key]["password"])) unset($tmp[$key]["password"]);
					}
					$return = $tmp;
				}
			}
	...
	function SQL_GET_NEXT_ID($_table)
	{
		if(isset($_table))
		{
			...
			$result = SQL_QUERY($conn, $query);
			$conn->close();
			if (!is_object($result) || $result->num_rows <= 0) return false;
	...
	function SQL_IS_ACTIVE($table, $id)
	{
		return SQL_CONTAINS_WHERE($_table=$table, $_where="id='" . $id . "' AND active = '1' ");
	}
?>
